    ...
    const handleGoogleLogin = async () => {
        try {
            const { error } = await supabase.auth.signInWithOAuth({
                provider: 'google',
                options: {
                    redirectTo: window.location.origin, // Redirects back to the same page/PWA
                    queryParams: {
                        access_type: 'offline',
                        prompt: 'consent',
                    },
                        ...
                        {!isRecoveryMode && (
                            <div className="pt-2 pb-2">
                                <button
                                    ...
                                    className="w-full py-3.5 bg-white border border-slate-200 text-slate-600 font-bold rounded-2xl hover:bg-slate-50 transition-all flex items-center justify-center gap-3 group relative overflow-hidden"
                                >
                                    ...
                                    <span className="group-hover:text-slate-800 transition-colors">Continuar com Google</span>
                                </button>
